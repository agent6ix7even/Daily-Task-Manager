...
## Автоматическая сборка установщика (GitHub Actions)

При каждом push в `main`, затрагивающем эту папку, GitHub Actions собирает проект на настоящей Windows-машине (без Wine и его ограничений в песочнице) и публикует на странице **Releases** репозитория:

- `Дневник задач Setup X.X.X.exe` — полноценный NSIS-установщик (ярлык в "Пуск", деинсталлятор).
- `Дневник задач X.X.X.exe` — portable-версия (без установки).

Прогресс и готовые файлы — во вкладке **Actions** репозитория, workflow "Build Desktop Installer (Windows)".
